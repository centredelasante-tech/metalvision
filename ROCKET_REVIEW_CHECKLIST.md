...
- [ ] **Patron récurrent — 6 occurrences confirmées (`INC-S06-07/08`, `INC-S07-04`, `INC-S08-01`, PR S05/Projets, PR S09/Cockpit) : un commit avec un parent Git à jour (`main` HEAD exact) peut quand même committer du contenu périmé si l'agent n'a pas resynchronisé son espace de travail local avant de créer la branche. Ne jamais se contenter de vérifier que le PR "part de main à jour" ; toujours diffuser fichier par fichier, y compris ceux non mentionnés dans la description du PR.** Traiter ce patron comme acquis et permanent : diff fichier par fichier de la totalité de la branche à chaque PR, indépendamment de toute consigne donnée en amont.
- [ ] **Variante du patron, découverte sur la PR S09 : même un fichier *existant* dont le diff part du bon parent (pas une copie intégrale périmée) peut réintroduire un bug déjà corrigé, si le hunk touche un bloc voisin de la zone réellement modifiée.** Sur `projets/[id]/page.tsx`, le remplacement légitime du calcul de risques par `computeProjectRisks` s'est accompagné d'une annulation non désirée d'`INC-S05-02` dans `handleVRSave()`, une fonction sans rapport avec la tâche demandée — probablement une régénération du fichier à partir d'une représentation interne périmée plutôt qu'un patch ciblé. **Conséquence pour la revue : ne pas se limiter à vérifier que les nouveaux fichiers sont corrects — pour chaque fichier *existant* modifié, comparer explicitement chaque hunk du diff à la dernière version connue-correcte sur `main`, même les hunks qui semblent sans rapport avec la demande de brief.**

---

*Dernière mise à jour : 12 juillet 2026, suite à la revue du PR frontend S09/Cockpit (6ᵉ occurrence du patron de copie locale périmée + variante inédite ayant réintroduit `INC-S05-02` dans un fichier existant) — voir ADR-MVP.md §9quindecies.*
