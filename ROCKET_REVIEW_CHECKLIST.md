...
- [ ] **Patron récurrent — 5 occurrences confirmées (`INC-S06-07/08`, `INC-S07-04`, `INC-S08-01`, et maintenant la PR S05/Projets) : un commit avec un parent Git à jour (`main` HEAD exact) peut quand même committer du contenu périmé si l'agent n'a pas resynchronisé son espace de travail local avant de créer la branche — l'ancienneté du *parent* ne garantit rien sur l'ancienneté du *contenu*. Ne jamais se contenter de vérifier que le PR "part de main à jour" ; toujours diffuser fichier par fichier, y compris ceux non mentionnés dans la description du PR.** La 5ᵉ occurrence (revue S05, 12 juillet 2026) est survenue malgré une consigne explicite de resynchronisation placée en tête du brief Rocket lui-même — la simple documentation/consigne écrite ne suffit donc plus à elle seule. **Ce patron doit maintenant être traité comme acquis et permanent tant que Rocket n'a pas confirmé un changement de process de son côté : prévoir systématiquement, à chaque PR, un diff fichier par fichier de la totalité de la branche (jamais seulement les fichiers annoncés) avant toute décision de fusion, indépendamment de toute consigne donnée en amont.**

---

*Dernière mise à jour : 12 juillet 2026, suite à la revue du PR frontend S05/Projets (5ᵉ occurrence du patron de copie locale périmée, + `INC-S05-02` nouveau bug sur `object_id` de `value_report_generated`) — voir ADR-MVP.md §9terdecies.*
