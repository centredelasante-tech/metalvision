...
## 29. Clôture — Agent d'aide MetalTrace V1 en production, rotation Gemini, hygiène secrets (8 août 2026)

**Contexte :** clôture documentaire de l'ensemble des GATE IA-1 à IA-CLOSE (Agent d'aide V1, migrations 15-17, activation du fournisseur LLM Production, hygiène des secrets Gemini/`.env`). Vérification finale en lecture seule (IA-CLOSE), sans changement de code, Supabase, Vercel ou secret dans ce GATE.

**1. Backend Agent — migrations Production**
Les trois migrations dédiées à l'Agent sont appliquées et confirmées présentes sur `dlbewgsoboaycbpypcus` (Production) : `15_assistant_interactions_audit_log` (`20260808060550`), `16_assistant_interactions_privilege_hardening` (`20260808061045`), `17_ai_distributed_rate_limit` (`20260808061652`).

**2. Surface d'accès — proxy legacy supprimé, Agent strictement read-only**
`/api/ai/chat-completion` (ancien proxy LLM générique) est définitivement absent du build Production (`404` confirmé à plusieurs reprises, y compris lors du smoke test final GATE D1). Seule route Agent exposée : `POST /api/assistant/ask`, strictement en lecture — aucune capacité d'écriture, aucun function-calling/tool exposé au modèle. Couverture limitée à une allowlist fixe de **8 écrans carbone** (`admin-verification-sessions`, `verifier-mrv`, `admin-carbon-inventory`, `admin-regroupement-distribution`, `admin-carbon-sales`, `admin-carbon-projects`, `admin-mrv-project`, `carbon-impact`) ; toute autre valeur rejetée en `400`. Le contexte remis au modèle est résolu exclusivement via des résolveurs prédéfinis (`CONTEXT_RESOLVERS`), avec le client Supabase authentifié de la requête — **RLS comme unique frontière d'accès aux données**, y compris pour un `objectId` arbitraire fourni par le client (hors RLS = résultat vide, jamais une fuite confirmant l'existence).

**3. Fournisseur LLM — Anthropic explicite, sans fallback**
Sélection du fournisseur strictement pilotée par la variable serveur `AI_LLM_PROVIDER` (jamais fournie ni influençable par le client HTTP), fail-closed sans exception : absente, hors de l'ensemble autorisé, ou clé API du fournisseur désigné absente → aucune sélection, `503 llm_not_configured`, jamais de repli implicite vers un autre fournisseur même si sa clé est présente. En Production, `AI_LLM_PROVIDER=anthropic` / `ANTHROPIC_API_KEY` (scope Production, ajoutées par l'utilisateur, GATE D1) — modèle fixé côté serveur, jamais accepté depuis le body HTTP : `claude-haiku-4-5-20251001`.

**4. Garde-fous opérationnels**
Rate limiting distribué (`ai_rate_limit_counters`, migration 17), persistant et atomique en base Postgres, écrit exclusivement via la fonction `SECURITY DEFINER check_ai_rate_limit()` — aucun privilège table-level à `authenticated`/`anon`. Journalisation d'audit (`assistant_interactions`, migrations 15-16) append-only et minimisée : jamais la question ni la réponse complètes ni le contenu du contexte métier, uniquement métadonnées bornées (utilisateur, écran, longueurs, modèle, tokens, latence, statut) ; RLS + privilèges restreints à `SELECT`/`INSERT` pour `authenticated`, aucun privilège `anon`/`PUBLIC`.

**5. Rotation `GEMINI_API_KEY` (Production et Preview)**
Ancienne clé Gemini exposée dans l'historique Git (`.env` anciennement suivi, voir §6 ci-dessous) rotée séparément en Production (GATE SEC-2B, validée par un test fonctionnel réel `analyze-photo` → `200`) et en Preview (GATE SEC-2C, branche `feature/agent-aide-v1-gate-ia-1`, Supabase METALVISION-DEMO, validée de la même façon). Inventaire de clôture SEC-2C : seul le projet Vercel `metalvision` dépend de `GEMINI_API_KEY` (deux scopes indépendants, tous deux rotés) ; le projet `metalvision-demo-web` n'en dépend pas. **L'ancienne clé Gemini a été révoquée par l'utilisateur lui-même côté fournisseur** (Google AI Studio) — action jamais effectuée par l'agent, conformément au mandat SEC-2C.

**6. Hygiène `.env` / Git**
`.env` retiré du suivi Git (`git rm --cached`), `.gitignore` durci, `.env.example` ajouté avec des valeurs exclusivement fictives (GATE SEC-1/SEC-2A, commit `0509802`, poussé sur `main` et déployé en Production). **Aucune purge d'historique Git effectuée** — l'ancien secret reste lisible dans l'historique du dépôt ; une éventuelle purge reste une décision séparée, explicitement non prise dans ce GATE ni dans aucun des précédents.

**7. Incident procédural — session Vercel concurrente pendant GATE D1**
Durant l'activation du fournisseur LLM en Production (GATE D1), une seconde session Claude (panneau de l'extension Chrome, hors du contrôle de cette conversation) est intervenue de façon autonome sur le même projet Vercel : elle a elle-même ajouté `AI_LLM_PROVIDER`/`ANTHROPIC_API_KEY` en scope Production et déclenché le redéploiement destiné à charger ces variables. Signalé à l'utilisateur en cours de GATE ; sur sa confirmation, le déploiement déjà déclenché (`dpl_EvX1zGVZNbF1conFdLdhUZ82LJvk`) a été adopté et vérifié de façon indépendante plutôt qu'un second redeploy concurrent. **Aucune régression constatée**, mais risque procédural réel (deux pilotes potentiels d'une même action Production). **Règle permanente adoptée à compter de ce GATE : une seule session/outillage est autorisée à piloter un GATE Production à la fois.** Avant toute modification Production future, la session pilote doit être désignée explicitement et confirmer : projet, environnement, branche, commit, Supabase cible — les autres sessions actives restant strictement en lecture seule pour la durée de l'action.

**8. Smoke test Production**
`POST /api/assistant/ask` (GATE D1, compte réel « Acier Laurentien Inc. », écran `carbon-impact`, question non sensible) → `HTTP 200`, `provider=anthropic`, `model=claude-haiku-4-5-20251001`, `status=success`, tokens et latence journalisés, rate limiter incrémenté, contexte correctement scopé par RLS, aucun secret dans la réponse ni les logs.

**9. Fingerprint canonique final (IA-CLOSE, lecture seule, 8 août 2026)**
Les 10 tables métier de référence (`organizations`, `aggregators`, `aggregator_memberships`, `platform_operators`, `carbon_commercialization_mandates`, `credit_issuances`, `credit_lots`, `credit_sales`, `verification_sessions`, `verification_outcomes`) montrent des **effectifs (`n`) strictement identiques** à la baseline IA-PROD-1 sur les 10 tables. Les hachages MD5 recalculés diffèrent de la baseline sur 6 des 10 tables (`organizations`, `credit_issuances`, `credit_lots`, `credit_sales`, `verification_sessions`, `verification_outcomes`) alors qu'ils correspondent exactement sur les 4 autres. La requête SQL exacte ayant produit les hachages baseline IA-PROD-1 n'a pas été conservée dans le dépôt ; une méthode plausible (`md5(string_agg(t::text,'|' ORDER BY id))`) a été reconstruite pour cette passe, sans certitude qu'elle soit identique à l'originale. Vérification de contenu indépendante (lecture seule) sur les 6 tables en écart : aucune ligne créée après le 2026-08-01 (bien avant la baseline IA-PROD-1 et tous les GATE Agent) sur les 5 tables sans colonne `updated_at` ; sur `organizations` (seule table du lot dotée d'une colonne `updated_at`), les 4 valeurs `updated_at` sont toutes elles aussi antérieures au 2026-08-01. **Conclusion retenue : absence d'indice de dérive réelle des données ; l'écart de hachage est très probablement imputable à une méthode de calcul non reconstituée à l'identique, pas à une modification de contenu.** Ceci reste néanmoins documenté comme un écart non résolu avec certitude absolue, faute de la requête originale.

**10. Portée — ce qui n'a pas été fait dans ce GATE (IA-CLOSE)**
Lecture seule stricte : aucun code, aucune migration, aucune variable Vercel/Supabase modifiée, aucune valeur de secret affichée ou comparée, aucun nouvel appel LLM (l'état Agent a été vérifié via éléments observables — déploiement, migrations, tables, dernier smoke test GATE D1 — sans rejouer `/api/assistant/ask`). Cette entrée ADR-MVP est une modification **locale** du dépôt de travail ; **aucun commit ni push `main` n'a été effectué** — GO distinct requis pour la committer.

**Conclusion :** l'Agent d'aide MetalTrace V1 est opérationnel en Production sur Anthropic (`claude-haiku-4-5-20251001`), strictement read-only, RLS-scopé, sans fallback, avec rate limiting distribué et audit minimisé append-only. Les clés Gemini Production/Preview sont rotées et l'ancienne clé révoquée par l'utilisateur ; `.env` n'est plus suivi par Git (historique non purgé, décision séparée). Un incident procédural de session Vercel concurrente est documenté avec une règle de gouvernance permanente adoptée pour le prévenir à l'avenir.
