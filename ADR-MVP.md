...
**État après cette session : les deux fichiers corrigés écrits sur `main`. Aucune migration requise (confirmé — les 3 policies RLS citées dans le brief couvrent les deux flux, comme vérifié avant l'écriture du brief). Reste : commits/push, puis reprise du test end-to-end (§54) à partir du bouton « Convertir en projet ».**
...
4. Tests end-to-end du parcours CCF complet (organisation → capacité → opportunité → invitation → mandat → projet → documents → logistique → rapport) — trou de portée E08-T01/T02 trouvé et documenté (§9sexvicies), brief Rocket rédigé, PR revue et corrigée (§9septvicies), `opportunities/page.tsx` et `projets/[id]/page.tsx` écrits sur `main`. Reste : commit/push, puis reprise du test à partir du bouton « Convertir en projet » sur l'opportunité « consolider Acton Vale ».
