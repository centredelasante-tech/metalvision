...
Deux scénarios validés en conditions réelles, chacun revérifié par requête SQL directe après action UI :
...
**Non testé, documenté explicitement (pas de réserve masquée) :** `release_credit_sale_lot` (libération d'un lot d'une vente encore `draft`, sans annuler la vente entière) n'a pas été exercé — aucun second lot disponible n'était accessible sans reconstruire un cycle d'émission complet, et l'utilisateur a choisi de ne pas le faire pour ce lot. Le code de la RPC a été lu et revu (même patron d'autorisation que les autres actions draft : `is_org_admin`/`is_platform_operator`, verrous `FOR UPDATE`), mais **pas exécuté en production**.
...
**6. Commit frontend**
- Lot 3 : cockpit de ventes (`/admin/carbon-sales`, page + lien hub `/admin`) — commit à pousser par l'utilisateur.

**7. Conclusion**
Le vertical slice du Lot 3 est validé en production sur le cycle principal complet (draft → lot → coût → confirmation → répartition calculée → règlement) et sur l'annulation d'une vente brouillon. La libération d'un lot (`release_credit_sale_lot`) reste non testée en production, faute d'inventaire disponible — gap documenté ici plutôt que masqué, à couvrir avant de considérer la matrice d'actions du Lot 3 aussi complète que celle du Lot 2. Un bug réel de production (rejet systématique de `confirm_credit_sale` par pg-safeupdate côté PostgREST, invisible à toute validation hors du chemin HTTP réel) a été détecté et corrigé pendant ce test — troisième bug réel consécutif trouvé uniquement par test de bout en bout en conditions réelles (après le bug RLS du Lot 2 et l'embed PostgREST ambigu du Lot 1), confirmant que cette étape n'est pas optionnelle pour ce projet. Les trois lots verticaux du MVP carbone commercial (inventaire, gouvernance, ventes) sont maintenant tous livrés et testés en production.
