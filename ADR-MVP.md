...
13. ~~Prochain écran selon la feuille de route 30-60-90 : S10 (`/admin`) complet.~~ — **en cours** : backend revu et corrigé (§9unvicies), brief Rocket rédigé (`Brief-Rocket-S10-Administration.md`). Reste à recevoir/revoir la PR Rocket, cherry-picker, déployer, puis tester en direct avec un compte temporairement élevé `is_platform_superadmin = true` (aucun compte de test actuel ne l'a).
