...
17. **Committer `Tranche0-Carbone-Architecture.md` et `supabase/carbon_migrations_proposed/`** (migration 01 révision 4 + script de test) pour disposer d'une référence de commit exacte de la version effectivement exécutée avec succès (22/22, §12) — non fait au moment de l'exécution, signalé en §12.
...
**Commit de référence — précision importante :** le dernier commit poussé au moment de l'exécution est `bc77b13` (« Tranche 0 carbone v4: 12 corrections... »). **Les fichiers réellement exécutés ne sont toutefois pas encore committés** : `Tranche0-Carbone-Architecture.md` est modifié dans l'arborescence de travail (`git status` : `M`) et l'intégralité de `supabase/carbon_migrations_proposed/` (migration 01 révision 4, script de test associé, et les 6 autres migrations à venir) est **non suivie par git** (`??` dans `git status`) — cohérent avec la consigne de l'utilisateur de ne jamais committer/pousser/fusionner ces fichiers automatiquement. **Aucune référence de commit exacte n'existe donc encore pour la version précisément exécutée.** Recommandation : committer `Tranche0-Carbone-Architecture.md` et `supabase/carbon_migrations_proposed/` immédiatement après cette validation, pour disposer d'un commit de référence traçable — cette entrée sera mise à jour avec le hash exact dès qu'il sera fourni.
...
- Fonction `can_view_carbon_event(p_actor_id, p_organization_id, p_aggregator_id, p_verification_session_id)` — version de base sans lecture de table (`SECURITY INVOKER`, récursion RLS évitée), destinée à être étendue par la migration 04 (`CREATE OR REPLACE`, même signature).
