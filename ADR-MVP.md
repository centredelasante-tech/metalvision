...
13. ~~Prochain écran selon la feuille de route 30-60-90 : S10 (`/admin`) complet.~~ — **en cours** : backend revu et corrigé (§9unvicies), PR Rocket revue et corrigée (§9duovicies) : `admin/page.tsx` (3 défauts corrigés) et `Sidebar.tsx` (reconcilié manuellement) intégrés, 7 fichiers périmés rejetés. Reste à pousser, fermer la branche/PR Rocket sans fusion, puis tester en direct avec un compte temporairement élevé `is_platform_superadmin = true` (aucun compte de test actuel ne l'a).
14. Après clôture de S10 : trancher la dette technique signalée en §9vicies/§11-12 (`AuthContext.tsx`/`useAuth()` — monter `AuthProvider` correctement ou retirer le code mort), comme convenu avec l'utilisateur pour la fin de cette session.
