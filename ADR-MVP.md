...
**Correction :** `supabase/migrations/20260712130000_incdata01_restore_mrv_tables.sql`, reconstruite fidèlement à partir de l'historique complet des 14 migrations pré-reset concernées (`20260609062345`, `20260609063000`, `20260613100000`, `20260628150000`, `20260630090000/090100/090200/090300/090400/150000`, `20260701010000/020000/030000`, `20260703200000`) : recrée les 8 tables vivantes avec leurs colonnes, index, triggers (`set_updated_at`, `set_transport_updated_at`, `compute_scan_event_hash` avec verrou consultatif, `verify_container_chain`) et policies RLS dans leur état final tel qu'il existait juste avant le reset. Dépendances déjà présentes en production vérifiées avant écriture (`is_company_member()`, `is_company_owner()`, `is_admin_from_auth()`, `companies`, `project_activity_logs`, toutes réapplquées lors du reset original). `clients` et `audit_learning_log` volontairement exclues (aucune référence dans le code vivant).

**Ceci est un incident distinct et sans lien avec le travail S01/S05/S09 de cette session** — la cause remonte au 10 juillet, avant toute revue visible dans cette session. Le blocage résiduel des 4 cartes KPI CCF (indépendant, `CCFDashboardSection.tsx` n'interroge aucune des tables MRV) reste à diagnostiquer séparément.

**Leçon retenue :** un fichier de migration portant un avertissement explicite « ne pas appliquer en production » doit être physiquement isolé du dossier `supabase/migrations/` (ex. `supabase/migrations-staging-only/` ou suppression après usage), pas seulement commenté — un avertissement en commentaire SQL n'empêche aucune exécution accidentelle via `supabase db push`.

**État après cette session : migration corrective écrite et vérifiée (parenthèses/dollar-quotes équilibrés), en attente de push par l'utilisateur puis de vérification post-application (requête d'inventaire fournie en fin de fichier de migration) et de nouveau test live de `/`.**
...
10. **Pousser `20260712130000_incdata01_restore_mrv_tables.sql`** (`supabase db push`), vérifier via la requête d'inventaire fournie en fin de fichier que les 8 tables existent, puis retester `/` en direct — la section CCF devrait alors se charger sans blocage.
11. Diagnostiquer séparément le blocage résiduel des 4 cartes KPI de `CCFDashboardSection.tsx` si celui-ci persiste après le push d'`INC-DATA-01` (cause probable : absence de `try`/`catch`/`finally` autour de `fetchAll`, `setLoading(false)` jamais atteint en cas d'erreur sur une requête CCF).
