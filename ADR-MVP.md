...
**État après cette session : `INC-DATA-01` résolu et validé en production.** Le blocage résiduel des 4 cartes KPI CCF sur `/` (indépendant — `CCFDashboardSection.tsx` n'interroge aucune des 8 tables ci-dessus) reste à diagnostiquer séparément ; reste aussi à confirmer que le domaine MRV (lots, conteneurs) fonctionne de nouveau sur `/lot-management`, `/qr-code-scanner`, etc., maintenant que ses tables existent.
...
10. ~~Pousser `20260712130000_incdata01_restore_mrv_tables.sql`~~ — **résolu** : appliqué en production après 2 corrections (voir §9novodecies), 8/8 tables confirmées par inventaire. Reste à retester `/` en direct pour confirmer que la section CCF se charge sans blocage.
